...
export class ProcessToolProvider implements ToolProvider {
  ...
  getTools(): Tool[] {
    return [
      ...
      {
        name: "proc_list",
        description: "Lists running processes on the remote system sorted by CPU usage",
        annotations: annotate({
      ...
      {
        name: "proc_kill",
        description: "Sends a signal to a running process by PID",
        annotations: annotate({
  ...
  handleTool(toolName: string, args: unknown): Promise<unknown> | undefined {
    switch (toolName) {
      ...
      case "proc_list":
        return this.procList(args);
      case "proc_kill":
        return this.procKill(args);
      default:
        return undefined;
    }
  }
  ...
  private async procList(args: unknown): Promise<unknown> {
    const { sessionId } = ProcListSchema.parse(args);
    const result = await this.deps.processService.execCommand(sessionId, "ps aux --sort=-%cpu");
    this.deps.metrics.recordCommand(result.durationMs, result.code === 0);
    logger.info("Process list retrieved", { sessionId });
    return result;
  }

  private async procKill(args: unknown): Promise<unknown> {
    const { sessionId, pid, signal } = ProcKillSchema.parse(args);
    const result = await this.deps.processService.execCommand(sessionId, `kill -${signal} ${pid}`);
    this.deps.metrics.recordCommand(result.durationMs, result.code === 0);
    logger.info("Process signaled", { sessionId, pid, signal });
    return result;
  }
