...
type Issue = { number: number; labels: Array<{ name: string }> };
type GhFailureResult = { status: number; stderr: string; stdout: string };
type GovernanceModule = {
  PROJECT_SCOPE_HINT: string;
  getRequiredLabels(taxonomy: TaxonomyFixture): Label[];
  validateLabels(actualLabels: Label[], taxonomy: TaxonomyFixture): string[];
  validateIssueTaxonomy(issues: Issue[], taxonomy: TaxonomyFixture): string[];
  validateProjectFields(
    fieldsPayload: { fields: Array<{ name: string }> },
